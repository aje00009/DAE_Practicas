...
- Modificación del `pom.xml` para añadir dependencias JWT (Alberto Jiménez Expósito)
- Creación de clase ``UtilJwt`` para gestión de tokens (Carlos Mayor Navarro)
- Creación de clase ``FiltroAutenticacionJwt`` para revisión de petición al controlador (Alberto Jiménez Expósito)
- Creacion de clase ``ServicioSeguridad`` para gestionar el acceso al sistema (Carlos Mayor Navarro)
- Creación de clase ``ServicioCredencialesUsuario`` para conectar a Spring Security con la BBDD (Alberto Jiménez Expósito)
- Creación de ``DAutenticacionUsuario`` como DTO para encapsular información de login (Carlos Mayor Navarro)
